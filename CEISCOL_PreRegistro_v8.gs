...
var INSTAGRAM_URL    = 'https://www.instagram.com/ceiscol';
...
function mostrarInstrucciones() {
  SpreadsheetApp.getUi().alert(
    ...
    '4. El correo se enviará automáticamente y el estado cambiará a "Link Enviado"\n\n' +
...
function actualizarDashboard() {
  SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Dashboard').getRange('C9').setFormula('=TEXT(NOW(),"DD/MM/YYYY HH:MM")');
...
function registrarEnvioMasivo(ss, cantidad, tipo) {
  ...
  h.appendRow([msgN, Utilities.formatDate(now,'America/Bogota','dd/MM/yyyy'),
...
// ── doGet — inyecta la URL del script en el HTML ─────────────────
function doGet() {
...
function darFormatoRegistros(h) {
  ...
  h.getRange(3,16,1000,1).setDataValidation(
    SpreadsheetApp.newDataValidation().requireValueInList(['Pendiente','Link Enviado','Confirmado','No responde']).build()
  );
...
function crearHojaMensajes(ss) {
  ...
  h.getRange(3,8,1000,1).setDataValidation(
    SpreadsheetApp.newDataValidation().requireValueInList(['✅ Enviado','❌ Error','⏳ Pendiente']).build()
  );
}
...
function crearHojaDashboard(ss) {
  ...
  h.setRowHeight(1,52);
  h.getRange(1,1,1,5).merge().setValue('🏆  DASHBOARD — POLLA MUNDIALISTA CEISCOL 2026 — PREMIO $1.000.000')
...
function guardarRegistro(ss, d) {
  ...
  h.appendRow([num,
    ...
    d.profesion, d.tipo_doc, d.num_doc, d.celular, d.email, fnac||'','','Pendiente']);
  if (fnac) {
...
function registrarMensaje(ss, d, num) {
  var h = ss.getSheetByName('Mensajes');
  var msgN = Math.max(h.getLastRow()-1, 0)+1, now = new Date();
  h.appendRow([msgN,
...
// ── Emails ───────────────────────────────────────────────────────
function btnIG() {
  return '<a href="https://www.instagram.com/ceiscol" target="_blank" '
    +'style="display:inline-block;background:linear-gradient(135deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888);'
    +'color:#fff;font-size:14px;font-weight:700;padding:12px 24px;border-radius:30px;text-decoration:none;margin-top:8px;">'
    +'📸 Seguir @ceiscol en Instagram</a>';
}
...
function enviarConfirmacion(d) {
  ...
  +'<div style="flex:1;background:#CE1126"></div></div>'
  ...
  +'<div style="background:#FFD700;padding:20px 28px;text-align:center;">'
  +'<p style="font-size:13px;font-weight:700;color:#003087;letter-spacing:1px;text-transform:uppercase;">🏆 Premio en efectivo</p>'
  +'<p style="font-size:36px;font-weight:900;color:#CE1126;line-height:1.1;">$1.000.000</p>'
  +'<p style="font-size:13px;color:#003087;font-weight:600;">Acierta el marcador Colombia 🇨🇴 vs Portugal 🇵🇹</p>'
  +'</div>'
  +'<div style="background:#fff;padding:28px 28px;">'
  +'<p style="font-size:15px;color:#1a1a2e;">Hola <b style="color:#003087;">'+nombre+'</b>,</p>'
  +'<p style="font-size:14px;color:#555;line-height:1.7;margin-top:10px;">Tu inscripción fue registrada con éxito. 🎉 Pronto recibirás el link para ingresar tu marcador.</p>'
  +'<div style="background:#f0f4ff;border:1.5px solid #003087;border-radius:12px;padding:18px;margin:20px 0;">'
  +'<p style="font-size:14px;font-weight:700;color:#003087;margin-bottom:10px;">📋 Requisitos para ganar:</p>'
  +'<p style="font-size:13px;color:#374151;line-height:1.8;">'
  +'✅ Ser colaborador de uno de nuestros clientes<br>'
  +'✅ Seguir <b>@ceiscol</b> en Instagram<br>'
  +'✅ Acertar el marcador exacto del partido</p>'
  +'</div>'
  +'<div style="text-align:center;margin:20px 0;">'
  +'<p style="font-size:13px;color:#555;margin-bottom:10px;">⚠️ <b>Requisito obligatorio:</b> debes seguirnos en Instagram</p>'
  +btnIG()
  +'</div>'
  ...
  +'<div style="flex:1;background:#003087"></div>'
  +'<div style="flex:1;background:#FFD700"></div></div>'
  +'</div></body></html>';
  ...
  MailApp.sendEmail({
    ...
    htmlBody: '<div style="font-family:Arial;max-width:540px;padding:20px;">'
...
function enviarLinkEmail(nombre, correo, link) {
  ...
  +'<div style="background:linear-gradient(135deg,#001233,#003087 50%,#8b0000);padding:36px 28px;text-align:center;">'
  +'<div style="font-size:56px;">⚽</div>'
  +'<h1 style="color:#FFD700;font-size:22px;font-weight:700;margin-top:10px;">¡Ya tienes tu link!</h1>'
  +'<p style="color:rgba(255,255,255,.8);font-size:13px;margin-top:5px;">Polla Mundialista CEISCOL 2026</p></div>'
  +'<div style="background:#FFD700;padding:16px 28px;text-align:center;">'
  +'<p style="font-size:13px;font-weight:700;color:#003087;">🏆 PREMIO: $1.000.000 EN EFECTIVO</p></div>'
  +'<div style="background:#fff;padding:28px 28px;">'
  +'<p style="font-size:15px;color:#1a1a2e;">Hola <b style="color:#003087;">'+nombre+'</b>,</p>'
  +'<p style="font-size:14px;color:#555;line-height:1.7;margin-top:10px;">¡Llegó tu momento! Ingresa tu marcador para Colombia vs Portugal 🇨🇴🆚🇵🇹</p>'
  +'<div style="text-align:center;margin:28px 0;">'
  +'<a href="'+link+'" style="display:inline-block;background:linear-gradient(110deg,#002566,#003087,#0047D4);color:#FFD700;font-size:16px;font-weight:900;padding:18px 36px;border-radius:16px;text-decoration:none;letter-spacing:1px;">⚽ INGRESAR MI MARCADOR</a>'
  +'</div>'
  +'<p style="font-size:13px;color:#888;text-align:center;">⏰ Regístralo antes del inicio del partido</p>'
  +'<div style="background:#f0f4ff;border:1.5px solid #003087;border-radius:12px;padding:16px;margin:20px 0;">'
  +'<p style="font-size:13px;color:#374151;line-height:1.8;">'
  +'✅ Ser colaborador de uno de nuestros clientes<br>'
  +'✅ Seguir <b>@ceiscol</b> en Instagram<br>'
  +'✅ Acertar el marcador exacto</p></div>'
...
<style>
html, body, div, p, h1, h2, span, label, input, select, button, form {
...
.hdr {
  ...
  padding: 36px 20px 52px; text-align: center; width: 100%;
}
.hdr-ico  { font-size: 44px; line-height: 1; margin-bottom: 10px; }
.hdr-badge {
...
.hdr h1 { font-size: 28px; font-weight: 900; color: #fff; line-height: 1.1; }
.hdr h1 span { color: #FFD700; display: block; }
.hdr-match {
...
.body { padding: 0 0 60px; width: 100%; }
.prize {
  margin: -26px 14px 0; position: relative; z-index: 20;
...
.prize-label { font-size: 11px; font-weight: 800; letter-spacing: 3px; text-transform: uppercase; color: #7A0018; margin-bottom: 4px; }
.prize-amount { font-size: 42px; font-weight: 900; color: #003087; line-height: 1; margin: 4px 0; }
.prize-sub { font-size: 13px; font-weight: 700; color: #001B4E; }
.prize-reqs {
  margin-top: 12px; background: rgba(0,48,135,.08); border-radius: 12px;
  padding: 10px 14px; font-size: 12px; color: #001B4E;
  line-height: 1.9; text-align: left; font-weight: 600;
  word-break: break-word;
}
.notice {
  margin: 16px 14px 0; background: #fff; border-radius: 16px;
  padding: 16px 18px; display: flex; align-items: center; gap: 12px;
  box-shadow: 0 4px 20px rgba(0,0,0,.08); border-left: 5px solid #003087;
  width: calc(100% - 28px);
}
.notice .ico { font-size: 26px; flex-shrink: 0; }
.notice p { font-size: 14px; color: #374151; line-height: 1.5; font-weight: 500; word-break: break-word; }
.notice b { color: #003087; }
.ig-btn-wrap { margin: 14px 14px 0; width: calc(100% - 28px); }
.ig-btn {
  ...
  box-shadow: 0 6px 20px rgba(188,24,136,.3); cursor: pointer;
  text-decoration: none; touch-action: manipulation;
  -webkit-tap-highlight-color: transparent;
}
.sec {
  margin: 16px 14px 0; background: #fff; border-radius: 20px;
  overflow: hidden; box-shadow: 0 2px 16px rgba(0,0,0,.07);
  width: calc(100% - 28px);
}
.sec-hdr { padding: 18px 18px 14px; border-bottom: 1px solid #F0F2F8; display: flex; align-items: center; gap: 10px; }
.sec-num {
  width: 28px; height: 28px; border-radius: 50%;
  background: #003087; color: #FFD700; font-size: 13px; font-weight: 900;
  display: flex; align-items: center; justify-content: center; flex-shrink: 0;
}
.sec-ttl { font-size: 16px; font-weight: 800; color: #111827; }
.f { padding: 0 18px; border-bottom: 1px solid #F0F2F8; width: 100%; }
.f:last-child { border-bottom: none; }
.f label {
  display: block; font-size: 13px; font-weight: 700; color: #6B7280;
  padding-top: 14px; margin-bottom: 7px;
  -webkit-user-select: none; user-select: none; word-break: break-word;
}
.f label .r { color: #EF4444; margin-left: 2px; }
.f input, .f select {
  display: block; width: 100%; max-width: 100%; height: 52px;
  padding: 0 14px; margin-bottom: 14px;
  background: #F8F9FC; border: 1.5px solid #DDE3F0; border-radius: 12px;
  font-size: 16px; font-family: inherit; font-weight: 500; color: #1A1D23;
...
.f input::placeholder { color: #A0AABA; font-weight: 400; }
.f select {
  ...
  background-size: 18px; background-color: #F8F9FC; padding-right: 42px; cursor: pointer;
}
.f select option { font-size: 16px; color: #1A1D23; background: #fff; }
.f select:disabled { opacity: .4; cursor: not-allowed; }
.pills { padding: 14px 18px 18px; }
.pgrid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
.pi { display: none; }
.pl {
  display: flex; flex-direction: column; align-items: center; justify-content: center;
  gap: 6px; min-height: 80px; padding: 14px 8px;
  background: #F8F9FC; border: 1.5px solid #DDE3F0; border-radius: 14px;
  cursor: pointer; text-align: center;
  -webkit-tap-highlight-color: transparent; touch-action: manipulation;
  user-select: none; -webkit-user-select: none;
  transition: border-color .13s, background .13s, box-shadow .13s;
}
.pl .e { font-size: 26px; line-height: 1; }
.pl .t { font-size: 13px; font-weight: 700; color: #6B7280; line-height: 1.3; }
.pi:checked + .pl { background: #EEF3FF; border-color: #003087; box-shadow: 0 0 0 3px rgba(0,48,135,.15); }
.pi:checked + .pl .t { color: #003087; }
.perr { display: none; font-size: 13px; color: #EF4444; padding: 4px 0 0; }
.pills.err .perr { display: block; }
.pills.err .pl { border-color: rgba(239,68,68,.4); }
.terms {
  margin: 14px 14px 0; width: calc(100% - 28px);
  background: #FFFBEB; border-radius: 16px; border: 1.5px solid #FDE68A;
  padding: 16px 18px; display: flex; gap: 12px; align-items: flex-start;
  box-shadow: 0 2px 10px rgba(0,0,0,.05);
}
.terms input[type=checkbox] {
  width: 24px; height: 24px; min-width: 24px; accent-color: #003087;
  cursor: pointer; touch-action: manipulation; margin-top: 2px; flex-shrink: 0;
}
.terms p { font-size: 13px; color: #4B5563; line-height: 1.65; word-break: break-word; }
...
.aok { background: #ECFDF5; border: 1.5px solid #10B981; color: #064E3B; }
.aer { background: #FEF2F2; border: 1.5px solid #EF4444; color: #7F1D1D; }
.btn-wrap { padding: 20px 14px 50px; width: 100%; }
.btn {
  display: flex; align-items: center; justify-content: center; gap: 10px;
  ...
  color: #FFD700; box-shadow: 0 8px 24px rgba(0,48,135,.4);
...
.spin {
  ...
  border: 3px solid rgba(255,215,0,.3); border-top-color: #FFD700;
  border-radius: 50%; animation: rot .7s linear infinite; flex-shrink: 0;
}
@keyframes rot { to { transform: rotate(360deg); } }
.foot {
  text-align: center; padding: 0 16px 36px;
  font-size: 12px; color: #9CA3AF; line-height: 1.7; word-break: break-word;
}
.foot b { color: #6B7280; font-weight: 700; }
</style>
</head>
<body>
...
<div class="hdr">
  ...
  <div class="hdr-badge">🏆 Copa Mundo 2026 · CEISCOL</div>
  <h1>Pre-Registro<span>Polla Mundialista</span></h1>
  <div class="hdr-match">🇨🇴 Colombia &nbsp;vs&nbsp; Portugal 🇵🇹</div>
</div>

<div class="body">

<div class="prize">
  <div class="prize-label">🏆 Premio en efectivo</div>
  <div class="prize-amount">$1.000.000</div>
  <div class="prize-sub">Al que acierte el marcador exacto 🎯</div>
  <div class="prize-reqs">
    ✅ Ser colaborador de nuestros clientes<br>
    ✅ Seguir <strong>@ceiscol</strong> en Instagram<br>
    ✅ Registrar el marcador antes del partido
  </div>
</div>

<div class="ig-btn-wrap">
...
<div class="notice">
  <div class="ico">📲</div>
  <p>Regístrate y <b>recibe el link exclusivo</b> para ingresar tu marcador y participar por <b>$1.000.000</b>.</p>
</div>

<form id="F" novalidate>

  <div class="sec">
    <div class="sec-hdr"><div class="sec-num">1</div><div class="sec-ttl">🏥 Laboratorio / Cliente</div></div>
    <div class="f">
      <label for="lab">Nombre del Laboratorio o Cliente <span class="r">*</span></label>
  ...
  <div class="sec">
    <div class="sec-hdr"><div class="sec-num">2</div><div class="sec-ttl">🔬 Tu Profesión</div></div>
  ...
  <div class="sec">
    <div class="sec-hdr"><div class="sec-num">3</div><div class="sec-ttl">👤 Datos Personales</div></div>
    <div class="f">
    ...
    <div class="f">
      ...
      <select id="dd"><option value="">Selecciona el día...</option></select>
    </div>
    <div class="f">
      <label for="dm">🗓️ Mes de Nacimiento <span class="r">*</span></label>
      <select id="dm">
    ...
    <div class="f">
      ...
      <select id="dy"><option value="">Selecciona el año...</option></select>
    </div>
    <div class="f">
...
<div class="ig-btn-wrap" style="margin-bottom:16px;">
  <a class="ig-btn" href="https://www.instagram.com/ceiscol" target="_blank">
    📸 Seguir @ceiscol — ¡Requisito para ganar!
  </a>
</div>
...
<div class="foot">
  ...
  📸 <a href="https://www.instagram.com/ceiscol" style="color:#bc1888;text-decoration:none;font-weight:700;">@ceiscol</a>
</div>
...
(function(){
  ...
  Object.keys(DPTS).sort().forEach(function(d){
    var o=document.createElement('option'); o.value=o.textContent=d; sd.appendChild(o);
  });
  var dd=document.getElementById('dd');
  for(var i=1;i<=31;i++){
    var o=document.createElement('option');
    o.value=o.textContent=(i<10?'0':'')+i; dd.appendChild(o);
  }
  var dy=document.getElementById('dy'), yr=new Date().getFullYear();
  for(var j=yr-16;j>=yr-85;j--){
    var o2=document.createElement('option'); o2.value=o2.textContent=j; dy.appendChild(o2);
  }
})();

document.getElementById('dpto').addEventListener('change',function(){
  var dval=this.value, sel=document.getElementById('ciu');
  sel.innerHTML='';
  if(!dval){sel.innerHTML='<option value="">Primero selecciona el departamento</option>';sel.disabled=true;return;}
  var arr=DPTS[dval]||[], def=document.createElement('option');
  def.value=''; def.textContent='Selecciona tu ciudad...'; sel.appendChild(def);
  arr.forEach(function(c){var o=document.createElement('option');o.value=o.textContent=c;sel.appendChild(o);});
  sel.disabled=false; this.classList.remove('err');
...
document.getElementById('F').addEventListener('submit',function(ev){
  ev.preventDefault(); ocultar();
  var lab=g('lab'),dpto=g('dpto'),ciu=g('ciu'),nom=g('nom'),ape=g('ape');
  var tdoc=g('tdoc'),ndoc=g('ndoc'),cel=g('cel'),mail=g('mail'),m2=g('mail2');
  var dd=g('dd'),dm=g('dm'),dy=g('dy'),prof='';
  ...
  if(!document.getElementById('trm').checked) return err('Debes aceptar los términos y condiciones.');
  busy(true);
  fetch(FURL,{
  ...
  .then(function(r){
    ...
    } else { err('Error del servidor: '+(r.msg||'intenta de nuevo.')); }
  })
  .catch(function(){err('No se pudo enviar. Verifica tu conexión e intenta de nuevo.');})
